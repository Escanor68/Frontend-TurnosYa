...
export const footballService = {
  // Campos/Canchas
  getFields: () => 
    footballApi.get<Field[]>('/fields').then(res => res.data),
  ...
  getNearbyFields: (lat: number, lng: number, radius: number) => 
    footballApi.get<Field[]>(`/fields/nearby`, {
      params: { lat, lng, radius }
    }).then(res => res.data),
  
  searchFields: (filters: FieldFilters) => 
    footballApi.get<Field[]>('/fields/search', {
      params: filters
    }).then(res => res.data),

  // Disponibilidad
  checkFieldAvailability: (fieldId: string, date: string) => 
    footballApi.get<FieldAvailability[]>(`/fields/${fieldId}/availability`, {
      params: { date }
    }).then(res => res.data),
  ...
  getBookings: (filters?: BookingFilters) => 
    footballApi.get<Booking[]>('/bookings', {
      params: filters
    }).then(res => res.data),
  ...
  addFieldReview: (fieldId: string, review: { rating: number; comment: string }) => 
    footballApi.post(`/fields/${fieldId}/reviews`, review).then(res => res.data),
